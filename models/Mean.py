...
class Mean(nn.Module):
    def __init__(self, config: MeanConfig, train_dataset: Dataset):
        super().__init__()
        self.config = config

        # Get the mean training dataset labels
        ppg_labels_all = []
        ppg_labels_diff_all = []
        ...
        self.ppg_labels_all_mean = ppg_labels_all.mean(axis=0).squeeze()
        self.ppg_labels_diff_all_mean = ppg_labels_diff_all.mean(axis=0).squeeze()
    ...
    def forward(self, nir_imgs: torch.Tensor, ppg_labels: torch.Tensor | None = None) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.config.out_dim == 1:
            ...
            logits = self.ppg_labels_diff_all_mean.repeat(batch_size, 1).to(device)
        else:
            # for seq2seq
            logits = self.ppg_labels_all_mean.repeat(batch_size, 1).to(device)
