    ...
    def get_test_data(self) -> dict[str, tuple[np.ndarray, np.ndarray]]:
        ...
        for subject_name, (nir_imgs, ppg_signal) in self.data.items():
            ...
            for idx in range(0, len(nir_imgs) - self.config.test_window_size + 1, self.config.test_window_stride):
                ppg_signal_window = ppg_signal[idx : idx + self.config.test_window_size]
                if np.any(ppg_signal_window < 1):
                ...
                ppg_signal_window_detrend = sig.detrend(ppg_signal_window)
                ppg_signal_window_spectrum = np.abs(np.fft.rfft(ppg_signal_window_detrend))
                freq = np.fft.rfftfreq(len(ppg_signal_window_detrend), d=1./self.config.video_fps)
                freq_range = np.logical_and(freq <= self.config.max_heart_rate / 60, freq >= self.config.min_heart_rate / 60)
                max_idx = np.argmax(ppg_signal_window_spectrum[freq_range])
                max_freq = freq[freq_range][max_idx]
                bpm = max_freq * 60
                start_idx_array.append(idx)
                bpm_array.append(bpm)
                spectrum_array.append(ppg_signal_window_spectrum)
